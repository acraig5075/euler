...
// split a number into the units component and the carry-over component
void split(int x, int *unit, int *tens)
	{
	*unit = x % 10;
	*tens = x / 10;
	}
...
int main(int argc, char *argv[])
	...
	for (int j = 49; j >= 0; j--, k--)
		...
		// split into units and carry-over components
		split(sum, &(total[k]), &carry);
		}

	total[0] = carry;

	// print only the first 10 digits of the total
	int tenth = 11 - digits(total[0]);
